...
var _ = Describe("Cluster-level RBAC export", func() {
	It("[CA-4b] Should export CRB with User subject referencing ServiceAccount", Label("cluster-admin"), func() {
		appName := "nginx-with-serviceaccount"
		namespace := "simple-nginx-nopv"
		serviceName := "my-" + appName
		saName := "nginx-sa"
		subject := "--user=system:serviceaccount:" + namespace + ":" + saName
		subjectName := "system:serviceaccount:" + namespace + ":" + saName
		...
		paths, err := NewScenarioPaths("crane-ca4b-*")
