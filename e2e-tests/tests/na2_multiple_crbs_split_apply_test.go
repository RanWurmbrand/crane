...
var _ = Describe("Namespace-admin cluster-level migration", func() {
	It("[NA-2] Should migrate workload with one CR and two CRBs using split apply", Label("namespace-admin"), func() {
		...
		Expect(RunPipeline(&adminRunner, namespace, adminPaths)).NotTo(HaveOccurred())
		...
		Expect(RunPipeline(&nonAdminRunner, namespace, nonAdminPaths)).NotTo(HaveOccurred())

		By("Verifying cluster resources are in failures directory (expected for namespace-admin)")
		err = ValidateDirResources(filepath.Join(nonAdminPaths.ExportDir, "failures", namespace), forbiddenResourcesPatterns)
		//cluster resources
		Expect(err).NotTo(HaveOccurred())

		By("Verifying no cluster resources in output _cluster directory")
		Expect(AssertNoClusterResources(filepath.Join(nonAdminPaths.OutputDir, "resources", "_cluster"))).NotTo(HaveOccurred())
